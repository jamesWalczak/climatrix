...
class TestDomainBuilder:
    ...
    def test_basic_sparse_domain_creation(self):
        ...
        domain = Domain.from_axes().lat(latitude=[1, 2, 3]).lon(longitude=[4, 5, 6]).sparse()
    ...
    def test_basic_dense_domain_creation(self):
        ...
        domain = Domain.from_axes().lat(latitude=[1, 2]).lon(longitude=[3, 4]).dense()
    ...
    def test_vertical_axis_in_sparse_domain(self):
        ...
        np.testing.assert_array_equal(domain.vertical.values, [10, 20, 30, 40, 50])
    ...
    def test_sparse_validation_mismatched_coordinates(self):
        ...
        with pytest.raises(ValueError, match="latitude and longitude arrays must have the same length"):
            Domain.from_axes().lat(latitude=[1, 2]).lon(longitude=[3, 4, 5]).sparse()
    ...
    def test_multiple_kwargs_error(self):
        ...
        with pytest.raises(ValueError, match="Exactly one keyword argument must be provided"):
    ...
    def test_slice_without_step_error(self):
        ...
        with pytest.raises(ValueError, match="Slice step must be specified"):
            Domain.from_axes().lat(latitude=slice(1, 10)).lon(longitude=[1]).sparse()
    ...
    def test_equivalence_with_from_lat_lon(self):
        ...
        domain_old = Domain.from_lat_lon(lat=np.array(lat_vals), lon=np.array(lon_vals), kind="sparse")
        
        # Create using new method
        domain_new = Domain.from_axes().lat(lat=lat_vals).lon(lon=lon_vals).sparse()
        
        assert domain_old.is_sparse == domain_new.is_sparse
        np.testing.assert_array_equal(domain_old.latitude.values, domain_new.latitude.values)
        np.testing.assert_array_equal(domain_old.longitude.values, domain_new.longitude.values)
